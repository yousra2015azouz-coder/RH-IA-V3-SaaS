...
def generate_interview_report(data: dict) -> bytes:
    ...
    main_title_style = ParagraphStyle('MainTitle', parent=styles['Heading1'], fontSize=22, textColor=PRIMARY, alignment=1, spaceAfter=20)
    ...
    header_table_data = []
    has_logo = False
    if logo_path and (logo_path.startswith("http") or os.path.exists(logo_path)):
        try:
    ...
    if has_logo:
        ...
        t_header = Table(header_table_data, colWidths=[18*cm])
        
    t_header.setStyle(TableStyle([('VALIGN', (0,0), (-1,-1), 'MIDDLE')]))
    elements.append(t_header)
    elements.append(Spacer(1, 0.5*cm))

    elements.append(Paragraph("COMPTE RENDU D'ENTRETIEN", main_title_style))
...
def generate_approval_pdf(data: dict) -> bytes:
    ...
    if has_logo:
        ...
        t_head = Table(header_data, colWidths=[0.1*cm, 18.9*cm])
    ...
    gen_data = [
        [Paragraph("Champ", th_style), Paragraph("Détails", th_style)],
        [Paragraph("Nom du collaborateur", label_style), Paragraph(f"<b>{data.get('nom_collaborateur') or 'N/A'}</b>", value_style)],
        [Paragraph("Date d'embauche", label_style), Paragraph(data.get("date_embauche") or "N/A", value_style)],
        [Paragraph("Date de naissance", label_style), Paragraph(data.get("date_naissance") or "N/A", value_style)],
        [Paragraph("Situation Familiale", label_style), Paragraph(data.get("situation_familiale") or "N/A", value_style)],
        [Paragraph("Personnes à charge", label_style), Paragraph(str(data.get("personnes_a_charge") or 0), value_style)]
    ]
    t_gen = Table(gen_data, colWidths=[9.5*cm, 9.5*cm])
    t_gen.setStyle(TableStyle([
        ('GRID', (0,0), (-1,-1), 0.5, ACCENT),
        ('BACKGROUND', (0,0), (-1,0), PRIMARY), # En-tête bleu
        ('BACKGROUND', (0,1), (0,-1), SECONDARY), # Colonne 1 grise
    ...
    org_data = [
        [Paragraph("Intitulé", th_style), Paragraph("Situation Proposée", th_style)],
        [Paragraph("Site (Succursale)", label_style), Paragraph(data.get("site") or job.get("site") or "N/A", value_style)],
        [Paragraph("Entité Organisationnelle", label_style), Paragraph(data.get("entity") or job.get("entity_organisationnelle") or "N/A", value_style)],
        [Paragraph("Fonction", label_style), Paragraph(data.get("job_title") or job.get("title") or "N/A", value_style)],
        [Paragraph("Emploi de référence", label_style), Paragraph(data.get("ref_job") or job.get("reference") or "N/A", value_style)],
        [Paragraph("Type de rémunération", label_style), Paragraph(data.get("type_remuneration") or "Direct", value_style)],
        [Paragraph("Grade", label_style), Paragraph(str(data.get("grade") or job.get("grade") or "N/A"), value_style)]
    ]
    t_org = Table(org_data, colWidths=[9.5*cm, 9.5*cm])
    ...
    sal_data = [
        [Paragraph("Rubrique", th_style), Paragraph("Situation Proposée (MAD)", th_style)],
        [Paragraph("Salaire de base", label_style), Paragraph(f"{data.get('salaire_base') or 0:,.2f}", value_style)],
        [Paragraph("Prime d'ancienneté", label_style), Paragraph("0.00", value_style)],
        [Paragraph("Indemnité Représentation", label_style), Paragraph("0.00", value_style)],
        [Paragraph("Indemnité Panier", label_style), Paragraph(f"{data.get('indemnite_panier') or job.get('indemnite_panier') or 550:,.2f}", value_style)],
        [Paragraph("Indemnité GSM", label_style), Paragraph(f"{data.get('indemnite_gsm') or job.get('indemnite_gsm') or 0:,.2f}", value_style)],
        [Paragraph("Indemnité Transport", label_style), Paragraph(f"{data.get('indemnite_transport') or job.get('indemnite_transport') or 550:,.2f}", value_style)],
        [Paragraph("Indemnité Caisse", label_style), Paragraph("0.00", value_style)],
        [Paragraph("Indemnité Spéciale", label_style), Paragraph("0.00", value_style)],
        [Paragraph("Prime de Performance", label_style), Paragraph("0.00", value_style)],
        [Paragraph("<b>Salaire Mensuel Brut</b>", label_style), Paragraph(f"<b>{data.get('salaire_mensuel_brut') or 0:,.2f}</b>", value_style)],
        [Paragraph("<b>Salaire Mensuel Net</b>", label_style), Paragraph(f"<b>{data.get('salaire_mensuel_net') or 0:,.2f}</b>", value_style)]
    ]
    t_sal = Table(sal_data, colWidths=[9.5*cm, 9.5*cm])
    t_sal.setStyle(TableStyle([
        ...
        ('BACKGROUND', (1, -2), (1, -2), colors.HexColor("#f8fafc"))  # Highlight Brut
    ]))
    ...
    adv_data = [
        [Paragraph("Rubrique", th_style), Paragraph("Situation Proposée", th_style)],
        [Paragraph("Primes Aïd / Scolarité", label_style), Paragraph(f"{data.get('prime_aid') or 2100:,.2f} MAD", value_style)],
        [Paragraph("Transport Personnel / Véhicule", label_style), Paragraph("Inclus (Panier/Transport)", value_style)],
        [Paragraph("Carte Carburant", label_style), Paragraph("Selon politique", value_style)],
        [Paragraph("Taux CIMR", label_style), Paragraph(f"{data.get('taux_cimr') or 6:.2f}%", value_style)],
        [Paragraph("<b>Salaire Annuel Garanti (SAG)</b>", label_style), Paragraph(f"<b>{data.get('salaire_annuel_garanti') or 0:,.2f} MAD</b>", value_style)],
        [Paragraph("<b>Salaire Annuel Total (SAT)</b>", label_style), Paragraph(f"<b>{data.get('salaire_annuel_total') or 0:,.2f} MAD</b>", value_style)]
    ]
    t_adv = Table(adv_data, colWidths=[9.5*cm, 9.5*cm])
